...
def get_error(signalList):
    ...
    for simulation in range(10):
        ...
        rawSignal = np.load('../groundTruth/Signals_Stim_'+str(simulation)+'.npy')
        ...
        totalDistance += np.sum(np.abs(rawSignal-signal)) #wasserstein_distance(rawSignal[0,0][timeIdx],signal[timeIdx])
